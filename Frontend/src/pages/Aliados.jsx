...
const handleShowModal = (aliado = null) => {
    if (aliado !== null) {
        setCurrentAliado(aliado);
        setEditIndex(aliado);
        
        // ✅ Si el aliado tiene un logo, lo mostramos
        if (aliado.logoAliado) {
            setPreviewLogo(aliado.logoAliado);
        } else {
            setPreviewLogo(null);
        }
    } else {
        setCurrentAliado({ nombreAliado: '', urlRedireccion: '' });
        setEditIndex(null);
        setPreviewLogo(null); // ✅ Restablece la previsualización cuando se agrega un nuevo aliado
    }
    setShowModal(true);
};
...
const handleFileChange = (e) => {
    ...
    if (file) {
        ...
        const imageURL = URL.createObjectURL(file); // ✅ Crea URL para previsualización
        setPreviewLogo(imageURL); // ✅ Actualiza la imagen de previsualización
        setCurrentAliado({ ...currentAliado, logoAliado: file });
    }
};
...
{previewLogo && (
    <div className="mt-2">
        <img src={previewLogo} alt="Previsualización del Logo" width="100" />
    </div>
)}
